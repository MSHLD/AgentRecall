...
3. 规范化路径末尾符合 `Codex/YYYY-MM-DD/<非空目录名>`；
...
当所有标题来源均为空或只得到通用的 `Untitled Session` 时，项目名称显示为本地化的未命名占位符，并附带根会话创建时间：
...
项目名称不直接使用随机目录 basename 作为最后回退；完整物理路径继续保留在项目行的 `title` 提示和会话详情中。
...
- 根会话数量；
- 唯一根会话来源；
- 唯一根会话的 `custom_title`、`original_title`、`first_question` 和创建时间。
...
4. 最终只改变 `ProjectSummary.label`，`ProjectSummary.path` 和 `environmentId` 保持不变。

这个实现使用现有数据库字段，不需要 schema migration。

### 渲染与重命名阶段

渲染器继续把 `ProjectSummary.label` 作为左侧项目名称，把 `ProjectSummary.path` 作为点击筛选值和悬停提示。
...
3. 如果创建时间缺失或追加分钟后仍然重复，最后追加任务目录 basename，确保同一环境内标签可区分。
...
不同环境中相同物理路径的现有环境名称消歧继续保留，且优先级高于任务标题的日期消歧。
