...
async function uninstall(options = {}) {
  const homeDir = options.homeDir || process.env.AGENT_SESSION_SEARCH_TEST_HOME || os.homedir();
  const messages = [];
  const errors = [];
  ...
  try {
    ...
    else messages.push(statusLine.status === "removed" ? "Removed the Agent-Session-Search Claude statusLine." : "Claude statusLine did not need changes.");
    ...
    const cacheFiles = [
      path.join(homeDir, ".agent-session-search", "update-check.json"),
      path.join(homeDir, ".agent-session-search", "update-install-status.json"),
      path.join(homeDir, ".agent-session-search", "app-process.json"),
